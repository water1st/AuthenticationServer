...
{
    internal class PublicKeyLoader : IKeyLoader
    {
        private readonly IOptions<KeyLoaderOptions> options;
        private readonly IKeyReader reader;

        public PublicKeyLoader(IOptions<KeyLoaderOptions> options, IKeyReader reader)
        {
        ...
        public async Task<RsaSecurityKey> LoadAsync()
        {
            ...
            var rsa = RSA.Create();
            rsa.ImportRSAPublicKey(key, out var _);
